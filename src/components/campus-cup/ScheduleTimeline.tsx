...
const stages = [
  ...
  { n: "02", title: "CHECK-IN", time: "16 Sep 2026 · Time 9:00 AM to 9:30 AM", text: "Participant Reporting, Check-in & Desk Registration." },
    ...
    <section id="schedule" ref={sectionRef} className="relative py-28 overflow-hidden bg-[#050505]">
      ...
      <div className="max-w-5xl mx-auto px-6 relative z-10">
        ...
        <div ref={listRef} className="relative ml-4 md:ml-12 space-y-0">
          ...
          {stages.map((s) => (
            <div key={s.n} className="ff-stage-item relative pl-16 pb-16 last:pb-0 group" style={{ opacity: 0 }}>
              ...
              <div className="absolute left-0 top-1.5 w-[32px] h-[32px] -translate-x-[0px] flex items-center justify-center">
                 ...
                 {/* Inner Dot */}
                 <div className="relative z-10 w-3 h-3 ff-hex-clip bg-white/20 group-hover:bg-[var(--color-ff-orange)] group-active:bg-[var(--color-ff-orange)] group-hover:shadow-[0_0_10px_var(--color-ff-orange)] group-active:shadow-[0_0_10px_var(--color-ff-orange)] transition-all" />
              </div>

              {/* Connecting horizontal dash */}
              <div className="absolute left-[32px] top-[17px] w-6 h-px bg-[var(--color-ff-orange)]/30" />

              <div className="flex flex-col md:flex-row md:items-start gap-6 border border-white/10 bg-white/[0.03] p-6 hover:bg-white/[0.06] active:bg-white/[0.08] hover:border-white/20 active:border-[var(--color-ff-orange)]/40 transition-all duration-300 relative overflow-hidden cursor-pointer select-none touch-manipulation">
                
                {/* Scanline effect on Hover / Touch */}
                <div className="absolute inset-0 -translate-y-full group-hover:translate-y-full group-active:translate-y-full transition-transform duration-1000 ease-in-out pointer-events-none bg-gradient-to-b from-transparent via-white/[0.04] to-transparent" />
                 
                {/* Top Active Bar (Appears on Hover / Touch) */}
                <div className="absolute top-0 left-0 right-0 h-1 bg-[var(--color-ff-orange)] opacity-0 group-hover:opacity-100 group-active:opacity-100 transition-opacity" />

                {/* Stage Number Watermark */}
                <div className="absolute right-4 bottom-4 font-display text-[6rem] leading-none text-white/[0.05] pointer-events-none">
                  {s.n}
                </div>

                <div className="flex-1 relative z-10">
                   <div className="flex items-center gap-3 mb-2">
                      <span className="font-sans text-[10px] tracking-[0.3em] font-bold uppercase border px-2 py-0.5 text-white border-white/20 group-hover:text-[var(--color-ff-orange)] group-active:text-[var(--color-ff-orange)] group-hover:border-[var(--color-ff-orange)]/40 group-active:border-[var(--color-ff-orange)]/40 group-hover:bg-[var(--color-ff-orange)]/10 group-active:bg-[var(--color-ff-orange)]/10 transition-colors">
                        PHASE {s.n}
                      </span>
                   </div>
                  <h3 className="font-display text-4xl uppercase tracking-wide mb-2 text-white group-hover:drop-shadow-[0_0_15px_rgba(255,107,0,0.3)] group-active:drop-shadow-[0_0_15px_rgba(255,107,0,0.3)] transition-all">{s.title}</h3>
                  <p className="font-sans font-bold tracking-[0.1em] text-sm mb-3 text-white group-hover:text-[var(--color-ff-orange)] group-active:text-[var(--color-ff-orange)] transition-colors">{s.time}</p>
                  <p className="font-sans text-sm text-white leading-relaxed max-w-xl">{s.text}</p>
                </div>
              </div>
            </div>
          ))}
