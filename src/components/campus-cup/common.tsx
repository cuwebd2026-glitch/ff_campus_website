...
export function SectionHeading({ code, title, side }: { code: string; title: ReactNode; side?: string }) {
  return (
    <div className="cc-section-heading">
      <p><span>{code}</span>{side ?? "CAMPUS CUP / SEASON 2"}</p>
      <h2>{title}</h2>
    </div>
  );
}
...
export function TournamentStats() {
  return (
    <div className="cc-stats">
      <div><strong>02</strong><span>Season</span></div>
      <div><strong>01</strong><span>College qualifier</span></div>
      <div><strong>14·09</strong><span>Match date</span></div>
      <div><strong>CU</strong><span>Campus</span></div>
    </div>
  );
}